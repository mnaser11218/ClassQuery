...
public class QuestionRepositoryWithBagRelationshipsImpl implements QuestionRepositoryWithBagRelationships {
    ...
    public Optional<Question> fetchBagRelationships(Optional<Question> question) {
        return question.map(this::fetchTags).map(this::fetchAnswers);
    }
    ...
    public List<Question> fetchBagRelationships(List<Question> questions) {
        return Optional.of(questions).map(this::fetchTags).map(this::fetchAnswers).orElse(Collections.emptyList());
    }
    ...
    Question fetchAnswers(Question result) {
        return entityManager
            .createQuery("select question from Question question left join fetch question.answers where question.id = :id", Question.class)
            .setParameter(ID_PARAMETER, result.getId())
            .getSingleResult();
    }

    List<Question> fetchAnswers(List<Question> questions) {
        HashMap<Object, Integer> order = new HashMap<>();
        IntStream.range(0, questions.size()).forEach(index -> order.put(questions.get(index).getId(), index));
        List<Question> result = entityManager
            .createQuery(
                "select question from Question question left join fetch question.answers where question in :questions",
                Question.class
            )
            .setParameter(QUESTIONS_PARAMETER, questions)
            .getResultList();
